...
static bool maybeReportUndefined(Undefined &sym, InputSectionBase &sec,
  ...
  if (sym.getName().starts_with("__llvm_prefetch_target_")) {
    dbgs() << "A prefetch for target: " << toString(sym) << " was rejected due to symbol undefined\n";
    return false;
  }
...
void RelocationScanner::processAux(RelExpr expr, RelType type, uint64_t offset,
  ...
  if (sym.getName().starts_with("__llvm_prefetch_target_")) {
    dbgs() << "A prefetch for target: " << toString(sym) << " was rejected due to relocation error\n";
    return;
  }
